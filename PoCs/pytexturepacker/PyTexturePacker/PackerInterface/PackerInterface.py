...
class PackerInterface(object):
    ...
    def pack(self, input_images, output_name, output_path="", input_base_path=None):
        ...
        for i, atlas in enumerate(atlas_list):
            ...
            packed_csv = atlas.dump_csv("%s%s" % (texture_file_name, self.texture_format), input_base_path)
            packed_atlas = atlas.dump_atlas("%s%s" % (texture_file_name, self.texture_format), input_base_path)
            ...
            Utils.save_csv(packed_csv, os.path.join(output_path, "%s.csv" % texture_file_name))
            Utils.save_atlas(packed_atlas, os.path.join(output_path, "%s.atlas" % texture_file_name))
